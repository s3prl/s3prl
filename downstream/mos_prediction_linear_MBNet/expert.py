...
class DownstreamExpert(nn.Module):
    def __init__(self, upstream_dim, downstream_expert, **kwargs):
        ...
        self.frame_weight = self.modelrc["frame_weight"]
    ...
    # Interface
    def forward(
        self, mode, features, means, system_names, moses, judge_ids, records, **kwargs
    ):
        ...
        if mode == "train":
            ...
            frame_scores, uttr_scores, bias_uttr_scores = self.model(
                features, lengths, judge_ids
            )
            frame_loss = self.objective(frame_scores, means.expand_as(frame_scores))
            uttr_loss = self.objective(uttr_scores, means).mean()
            bias_loss = self.objective(bias_uttr_scores, moses).mean()
