...
class Model(nn.Module):
    def __init__(self, input_dim, output_class_num, rnn_layers, hidden_state, **kwargs):
        super(Model, self).__init__()
        
        # init attributes
        self.use_rnn = (rnn_layers > 0)
        if self.use_rnn:
            self.rnn = nn.LSTM(input_dim, hidden_size, num_layers=rnn_layers)
