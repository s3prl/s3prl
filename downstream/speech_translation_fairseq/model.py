...
class Transformer(nn.Module):
    ...
    def __init__(self, vocab_size, padding_idx, share_projection=True, **config):
        super().__init__()
    ...
    def forward(self, features, features_length, decoder_input_idx):
        ...
        predict = self.output_projection(logit)
        return predict
    ...
    def incremental_decode(self, features, features_length, start_ids, max_len):
        ...
        for i in range(max_len):
            tgt_mask = self.model.generate_square_subsequent_mask(output_ids.size(0)).to(features.device)
            tgt = self.pos_encoding(self.embedding(output_ids))
