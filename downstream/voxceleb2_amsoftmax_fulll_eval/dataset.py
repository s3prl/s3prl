...
class SpeakerVerifi_plda(Dataset):
    def __init__(self, vad_config, file_path, meta_data, max_timestep=None):
        ...
        for key in self.root_key:
            ...
            cache_path = f"./downstream/voxceleb2_amsoftmax_fulll_eval/cache_wav_paths/cache_{key}.p"
            ...
            if os.path.isfile(cache_path):
                ...
                for speaker_id in list(cache_wavs_dict.keys()):
                    for wavs in cache_wavs_dict[speaker_id]:
                        utterance_id = str(speaker_id / wavs).replace(".wav","").replace("/","-")
                ...
                for speaker in tqdm.tqdm(speaker_dirs):
                    ...
                    for wav in wav_list:
                        ...
                        if length > self.vad_c['min_sec']: 
                            utterance_id = str(speaker_dir/wav).replace(".wav","").replace("/","-")
    ...
    def __getitem__(self, idx):
        wav, _ = torchaudio.load(str(audio_path[0]))
        # wav, _ = apply_effects_file(str(audio_path[0]), EFFECTS)
        wav = wav.squeeze(0)
        length = wav.shape[0]
...
class SpeakerVerifi_test_plda(Dataset):
    def __init__(self, vad_config, file_path, meta_data):
