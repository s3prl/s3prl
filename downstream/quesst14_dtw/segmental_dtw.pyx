...
ctypedef double (*func_pointer)(double[:], double[:])


@cython.boundscheck(False)
@cython.wraparound(False)
cpdef double segmental_dtw(double[:, :] query, double[:, :] doc, str metric_name):
    '''
    ...
    cdef double[:,:] dist_mat = distance_mat(query, doc, metric_name)
...
cdef double[:,:] distance_mat(double[:, :] query, double[:, :] doc, str metric_name):
    cdef int N = query.shape[0]
    cdef int M = doc.shape[0]

    cdef func_pointer distance_metric
    if (metric_name == "L1"):
        distance_metric = l1_norm
    elif (metric_name == "L2"):
        distance_metric = l2_norm
    elif (metric_name == "cos"):
        distance_metric = negative_log_dot
    ...
    for i in range(N):
        for j in range(M):
            dist_mat[i, j] = distance_metric(query[i], doc[j])
    ...
    for i in range(N):
        ...
        for j in range(M):
            dist_mat[i, j] = (dist_mat[i, j] - dist_min) / (dist_max - dist_min + 1e-6)
