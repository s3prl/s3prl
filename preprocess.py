...
from utils.audio import extract_feature
# sys.path.insert(0, '..')
...
def acoustic_preprocess(args, tr_set, encode_table, output_dir, bpe_dir, dim):
	...
	for s in tr_set:
		...
		tr_x = Parallel(n_jobs=args.n_jobs)(delayed(extract_feature)(str(file),feature=args.feature_type,dim=args.feature_dim,\
				cmvn=args.apply_cmvn,delta=args.apply_delta,delta_delta=args.apply_delta_delta,save_feature=os.path.join(cur_path,str(file).split('/')[-1].replace('.flac',''))) for file in tqdm(todo))
...
def main():
	...
	encode_table = None
	output_dir = None
	dim = args.feature_dim*(1+args.apply_delta+args.apply_delta_delta)
