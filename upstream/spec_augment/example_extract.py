# -*- coding: utf-8 -*- #
"""*********************************************************************************************"""
#   FileName     [ upstream/spec_augment/example_extract.py ]
...
Upstream = getattr(importlib.import_module('hubconf'), 'spec_augment')
model = Upstream(refresh=True).to(device)


# below is an example of using a local checkpoint
try:
    path = './s3prl/spec_augment/logMelBase-AdamW-b32-1m-960hr-drop1/states-1000000.ckpt'
    Upstream_local = getattr(importlib.import_module('hubconf'), 'spec_augment_local')
