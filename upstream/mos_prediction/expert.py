...
class UpstreamExpert(UpstreamBase):
    ...
    def _get_mos_upstream(self):
        mos_upstream = torch.hub.load('s3prl/s3prl', self.upstream_type)
    ...
    def _get_mos_downstream(self):
        mos_downstream = MosDownstream(
            ...
            projector_dim = self.checkpoint['Config']['downstream_expert']['modelrc']['projector_dim'],
            clipping = self.checkpoint['Config']['downstream_expert']['modelrc']['clipping'],
            attention_pooling = self.checkpoint['Config']['downstream_expert']['modelrc']['attention_pooling']
        )
