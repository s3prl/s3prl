# -*- coding: utf-8 -*- #
"""*********************************************************************************************"""
#   FileName     [ upstream/byol/example_extract.py ]
...
path = './result/pretrain/simsiam-init/states-epoch-1000.ckpt'
Upstream_local = getattr(importlib.import_module('hubconf'), 'byol_local')
model = Upstream_local(ckpt=path).to(device)

# example_wavs: a batch of audio in wav:  (batch_size, wav_time_step)
example_wavs = [torch.ones(160000, dtype=torch.float).to(device) for _ in range(16)]
