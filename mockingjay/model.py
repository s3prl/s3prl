...
class MockingjayConfig(object):
    ...
    def __init__(self, config):
        ...
        self.embedding_size = config['mockingjay']['embedding_size']
...
class MockingjayForMaskedAcousticModel(MockingjayInitModel):
    ...
    def forward(self, spec_input, pos_enc, mask_label=None, attention_mask=None, spec_label=None, head_mask=None):
        ...
        return pred_spec
class MockingjayForMaskedAcoustic_SpectOrderModel(MockingjayInitModel):
    """Mockingjay model with the masked acoustic modeling head.
    This module comprises the Mockingjay model followed by the masked acoustic modeling head.
    ...
    """
    def __init__(self, config, input_dim, output_dim, output_attentions=False, keep_multihead_output=False):
        super(MockingjayForMaskedAcoustic_SpectOrderModel, self).__init__(config)
        self.output_attentions = output_attentions
        self.Mockingjay = MockingjayModel(config, input_dim, output_attentions=output_attentions,
                                      keep_multihead_output=keep_multihead_output)
        self.SpecHead = MockingjaySpecPredictionHead(config, output_dim if output_dim is not None else input_dim)
        self.OrderHead= SpecOrderHead(config)
        self.apply(self.init_Mockingjay_weights)
        self.loss = nn.L1Loss() 
        self.order_loss = nn.BCELoss()

    def forward(self, spec_input, pos_enc, mask_label=None, attention_mask=None, spec_label=None, order_label=None, head_mask=None):
        outputs = self.Mockingjay(spec_input, pos_enc, attention_mask,
                            output_all_encoded_layers=False,
                            head_mask=head_mask)
        if self.output_attentions:
...
class SpecOrderHead(nn.Module):
    def __init__(self, config):
        super(SpecOrderHead, self).__init__()
        self.dense = nn.Linear(config.hidden_size,1)
        if isinstance(config.hidden_act, str) or (sys.version_info[0] == 2 and isinstance(config.hidden_act, unicode)):
            self.transform_act_fn = ACT2FN[config.hidden_act]
        else:
            self.act = config.hidden_act
        self.total_length = torch.tensor(640).cuda()
        self.dense2= nn.Linear(640, 2)
        self.softmax = nn.Softmax(-1)
    def forward(self, hidden_states):
        score = self.dense(hidden_states)
        score = score.squeeze(2)
        length=score.shape[1]
        pad_seq = torch.cat((score,torch.zeros([score.shape[0],self.total_length - length]).cuda()),dim=1)
        linear = self.dense2(pad_seq)
        prob = self.softmax(linear)
        return prob

class MockingjayAlbertEncoder(nn.Module):
    def __init__(self, config, output_attentions=False, keep_multihead_output=False):
        super(MockingjayAlbertEncoder, self).__init__()
        self.output_attentions = output_attentions
        self.config = config
        self.layer = MockingjayLayer(config, output_attentions=output_attentions,
                                  keep_multihead_output=keep_multihead_output)
        # self.layer = nn.ModuleList([copy.deepcopy(layer) for _ in range(config.num_hidden_layers)])
...
class MockingjayModelALbert(MockingjayInitModel):
    """Mockingjay model ("Bidirectional Embedding Representations from a Transformer").
    ...
    """
    def __init__(self, config, input_dim, output_attentions=False, keep_multihead_output=False):
        super(MockingjayModelALbert, self).__init__(config)
        self.output_attentions = output_attentions
        self.input_representations = MockingjayInputRepresentations(config, input_dim)
        self.encoder = MockingjayAlbertEncoder(config, output_attentions=output_attentions,
                                           keep_multihead_output=keep_multihead_output)
        self.apply(self.init_Mockingjay_weights)
...
class MockingjayForMaskedAcoustic_SpectOrder_Mask_By_Probability_Model(MockingjayInitModel):
    """ Mockingjay model with the masked acoustic modeling head.
        This module comprises the Mockingjay model followed by the masked acoustic modeling head.
    ...
    """
    def __init__(self, config, input_dim, output_dim, output_attentions=False, keep_multihead_output=False):
        super(MockingjayForMaskedAcoustic_SpectOrder_Mask_By_Probability_Model, self).__init__(config)
        self.output_attentions = output_attentions
        self.Mockingjay = MockingjayModel(config, input_dim, output_attentions=output_attentions,
                                      keep_multihead_output=keep_multihead_output)
        self.SpecHead = MockingjaySpecPredictionHead(config, output_dim if output_dim is not None else input_dim)
...
class MockingjayForMaskedAcoustic_SpectOrder_Mask_By_Probability_Model_albert(MockingjayInitModel):
    """ Mockingjay model with the masked acoustic modeling head.
        This module comprises the Mockingjay model followed by the masked acoustic modeling head.
    ...
    """
    def __init__(self, config, input_dim, output_dim, output_attentions=False, keep_multihead_output=False):
        super(MockingjayForMaskedAcoustic_SpectOrder_Mask_By_Probability_Model_albert, self).__init__(config)
        self.output_attentions = output_attentions
        self.Mockingjay = MockingjayModelALbert(config, input_dim, output_attentions=output_attentions,
                                      keep_multihead_output=keep_multihead_output)
        self.SpecHead = MockingjaySpecPredictionHead(config, output_dim if output_dim is not None else input_dim)
        self.OrderHead= SpecOrderHead(config)
        self.apply(self.init_Mockingjay_weights)
