...
def acoustic_preprocess(args, dim):
	...
	output_dir = os.path.join(args.output_path,'_'.join(['mosi_nonstandard',str(args.feature_type)+str(dim)]))
	...
	np.random.seed(42)
	data_count = len(df)
	permute = np.random.permutation(data_count)
	belong = np.array(['nan'] * data_count, dtype=np.object)
	train_end = int(data_count * 0.6)
	test_end = train_end + int(data_count * 0.25)
	belong[permute[:train_end]] = 'train'
	belong[permute[train_end:test_end]] = 'test'
	belong[permute[test_end:]] = 'dev'
	df.insert(len(df.columns), 'set', belong)

	# move each npy to split directory
	for split in ['train', 'dev', 'test']:
		os.mkdir(os.path.join(output_dir, split))
	for index, row in df.iterrows():
		source_path = os.path.join(output_dir, row.file_path)
		target_path = os.path.join(output_dir, row.set)
