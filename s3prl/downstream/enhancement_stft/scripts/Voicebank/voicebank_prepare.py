...
def download_vctk(voicebank_archive, destination, tmp_dir=None, device="cpu"):
    """Download dataset and perform resample to 16000 Hz.

    Arguments
    ---------
    voicebank_archive: str
        The zip file of voicebank
    destination : str
    ...
    logger.info("Unzipping " + voicebank_archive)
    shutil.unpack_archive(voicebank_archive, tmp_dir, "zip")
    tmp_dir = str(Path(tmp_dir) / "voicebank")
...
if __name__ == "__main__":
    ...
    parser.add_argument("voicebank_archive", help="voicebank.zip")
    parser.add_argument("destination")
    args = parser.parse_args()

    Path(args.destination).mkdir(exist_ok=True, parents=True)
    download_vctk(args.voicebank_archive, args.destination)
