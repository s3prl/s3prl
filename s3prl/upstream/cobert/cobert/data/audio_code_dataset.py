...
class AudioCodeDataset(FairseqDataset):
    def __init__(
        ...
        if code_paths is not None:
            ...
            if store_labels:
                self.code_list = [load_label(p, inds, tot) for p in code_paths]
            else:
                self.code_offsets_list = [
                    load_label_offset(p, inds, tot) for p in code_paths
                ]
            assert code_processors is not None
            self.code_processors = code_processors
            verify_label_lengths(
    ...
    def get_code(self, index):
        if self.store_labels:
            label = self.code_list[0][index]
        else:
            with open(self.code_paths[0]) as f:
                offset_s, offset_e = self.code_offsets_list[0][index]
                f.seek(offset_s)
                label = f.read(offset_e - offset_s)

        if self.code_processors is not None:
            label = self.code_processors[0](label)
        return label
    ...
    def collater(self, samples):
        ...
        if self.code_paths is not None:
            ...
            code_list, _, _ = self.collater_label(
                targets_by_code, audio_size, audio_starts
            )

        if self.code_input:
            # use code as 'audio' input
            if self.code_paths is not None:
                collated_audios = code_list[0]
            else:
                collated_audios = targets_list[0]
