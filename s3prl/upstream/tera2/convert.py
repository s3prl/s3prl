...
import s3prl
from s3prl.upstream.tera2.tera_model import (
    TeraConfig,
    TeraModel,
    TeraPretrainingConfig,
)
...
def load_converted_model(ckpt: str):
    ...
    task_cfg = merge_with_parent(TeraPretrainingConfig, ckpt_state["task_cfg"])
    model_cfg = merge_with_parent(TeraConfig, ckpt_state["model_cfg"])
    model = TeraModel(model_cfg, task_cfg, ckpt_state["dictionaries_symbols"])
