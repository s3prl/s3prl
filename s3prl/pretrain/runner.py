...
class Runner:
    ...
    def __init__(self, args, config):
        ...
        self.init_ckpt = (
            torch.load(self.args.past_exp, map_location="cpu")
            if self.args.past_exp
            else {}
        )
    ...
    def _get_upstream(self):
        ...
        Upstream = getattr(
            importlib.import_module(module_path), "UpstreamPretrainExpert"
        )
        upstream = Upstream(
        ...
        while pbar.n < pbar.total:
            for data in tqdm(dataloader, dynamic_ncols=True, desc="train"):
                ...
                grad_norm = torch.nn.utils.clip_grad_norm_(
                    self.upstream.model.parameters(),
                ...
                # logging
                if (
                    global_step % self.config["runner"]["log_step"] == 0
                    or pbar.n == pbar.total - 1
                ):
                    # log loss
                    self.logger.add_scalar(
                        f"{prefix}loss", all_loss, global_step=global_step
                    )
                    # log lr
                    ...
                    # log norm
                    self.logger.add_scalar(
                        f"{prefix}gradient norm", grad_norm, global_step=global_step
                    )
                ...
                if (
                    global_step % self.config["runner"]["save_step"] == 0
                    or pbar.n == pbar.total - 1
                ):
                    ...
                    name = (
                        f"states-epoch-{n_epochs}.ckpt"
                        if pbar.n == pbar.total - 1 and n_epochs > 0
