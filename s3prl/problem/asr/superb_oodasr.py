...
from s3prl.dataio.corpus.commonvoice import CommonVoice
...
def prepare_commonvoice(
    ...
    """
    Prepare Commonvoice for ASR following :obj:`SuperbOODASR.prepare_data` format.
    See :obj:`Commonvoice` for the arguments usage
    """


    bucket_size = 32
    corpus = CommonVoice(tokenizer, bucket_size, dataset_root)
    
    train_path = dataset_root + "/train"
    valid_path = dataset_root + "/dev"
    test_paths = dataset_root + "/test"
...
def prepare_common_tokenizer(
    ...
    """
    Build the tokenizer following :obj:`SuperbOODASR.build_tokenizer` format
...
class SuperbOODASR(ASR):
    ...
    def prepare_data(
        ...
        By default call :obj:`prepare_commonpeech` with :code:`**prepare_data`

        Args:
            prepare_data (dict): same in :obj:`default_config`, support arguments in :obj:`prepare_commonvoice`
